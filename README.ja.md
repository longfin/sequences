...
すべてのデータはブラウザ（IndexedDB）にのみ保存されます。独自のサーバーは
なく、アカウントも不要です。同じ本をiPadでも見たいときは、ログインして
シーケンスとサムネイルをデバイス間で同期できます。その場合もオリジナルの
写真は取り込んだデバイスの外に出ません。
...
InDesignのようなレイアウトソフトでも見開きは見られます — ただし、ドキュメント
設定やマスター、スタイルなど、まだ必要のないものと一緒に。シーケンスの段階で
やりたいのは、ただ写真をページに置いて眺めることだけ。Sequencesはそのための
最小限のツールです。タブを開いて、写真をドラッグして、流れがしっくりくるまで
並べ替える。インストールも設定も要りません。
